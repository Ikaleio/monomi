...
function errorCode(error: unknown, timedOut: boolean): CheckErrorCode {
  ...
  const message = error instanceof Error ? `${error.message} ${(error.cause as Error | undefined)?.message ?? ""}`.toLowerCase() : ""
  if (message.includes("dns") || message.includes("getaddrinfo") || message.includes("name not resolved")) return "DNS_ERROR"
...
function fail(start: number, code: CheckErrorCode, message: string, statusCode?: number): CheckOutcome {
  return {
    success: false,
  ...
  try {
    for (let redirects = 0; redirects <= 5; redirects += 1) {
      const response = await fetch(currentUrl, {
        method,
        headers: { ...monitor.headers, "User-Agent": `Monomi/${packageJson.version}` },
      ...
      if (
        ...
        if (redirects === 5) {
          return fail(start, "TOO_MANY_REDIRECTS", "重定向次数超过 5 次", response.status)
        }
      ...
      if (monitor.keyword) {
        ...
        if (reader) {
          while (true) {
            ...
            if (bytes + value.byteLength > MAX_RESPONSE_BYTES) {
              await reader.cancel()
              return fail(start, "RESPONSE_TOO_LARGE", "响应正文超过 1 MiB", response.status)
            }
        ...
        if (!text.includes(monitor.keyword)) {
          return fail(start, "KEYWORD_MISSING", "响应中未找到关键字", response.status)
        }
      } else {
        await response.body?.cancel()
      }
