...
export function createConfigTransferRoutes(deps: AppDeps) {
  return new Hono<AppEnv>()
    .get("/export", async (c) => {
      ...
      if (!appSettings)
        throw new ApiError(500, "SETTINGS_MISSING", "系统设置尚未初始化")
      const monitorRows = await deps.db.query.monitors.findMany({
      ...
      const mappings =
        await deps.db.query.monitorNotificationChannels.findMany()
      const statusRows = await deps.db.query.statusPageMonitors.findMany({
        orderBy: asc(statusPageMonitors.sortOrder),
      })
      const monitorById = new Map(
        monitorRows.map((monitor) => [monitor.id, monitor.name])
      )
      const document: ConfigDocument = {
        ...
        notifications: channels.map((channel) => ({
          ...
          monitorNames: channel.allMonitors
            ? null
            : mappings
                .filter((mapping) => mapping.channelId === channel.id)
                .map((mapping) => monitorById.get(mapping.monitorId))
                .filter((name): name is string => Boolean(name)),
        })),
        statusPageMonitorNames: statusRows
          .map((row) => monitorById.get(row.monitorId))
          .filter((name): name is string => Boolean(name)),
      }
    ...
    .post("/import", async (c) => {
      ...
      if (length > 1024 * 1024)
        throw new ApiError(413, "CONFIG_TOO_LARGE", "配置不得超过 1 MiB")
      let raw: unknown
      try {
        raw = await c.req.json()
      } catch {
        throw new ApiError(400, "INVALID_JSON", "配置必须是有效 JSON")
      }
      const document = configDocumentSchema.parse(raw)
      const now = deps.now?.() ?? new Date()
      const heartbeatTokens: Array<{
        monitorName: string
      ...
      try {
        deps.db.transaction((tx) => {
          ...
          tx.update(settings)
            .set({ ...document.settings, logoPath: null, updatedAt: now })
            .where(eq(settings.id, 1))
            .run()
          ...
          for (const input of document.monitors) {
            ...
            const heartbeatToken =
              input.type === "heartbeat" ? randomToken() : null
            ...
            tx.insert(monitors)
              .values({
                ...
                heartbeatTokenHash: heartbeatToken
                  ? Bun.CryptoHasher.hash("sha256", heartbeatToken, "hex")
                  : null,
          ...
          for (const input of document.notifications) {
            ...
            tx.insert(notificationChannels)
              .values({
                ...
                enabled: input.enabled,
                allMonitors: input.monitorNames === null,
                createdAt: now,
                updatedAt: now,
              })
              .run()
            for (const name of input.monitorNames ?? []) {
              const monitorId = idsByName.get(name)
              if (!monitorId)
                throw new Error(`通知映射引用不存在的监视器：${name}`)
              tx.insert(monitorNotificationChannels)
                .values({ monitorId, channelId })
                .run()
            }
          }
          document.statusPageMonitorNames.forEach((name, sortOrder) => {
            const monitorId = idsByName.get(name)
            if (!monitorId) throw new Error(`状态页引用不存在的监视器：${name}`)
