...
export function createOverviewRoutes(deps: AppDeps) {
  return new Hono<AppEnv>().get("/", async (c) => {
    ...
    const [allMonitors, recentChecks, failedDeliveries, appSettings] = await Promise.all([
    ...
    const averageLatencyMs = successful.length
      ? Math.round(successful.reduce((sum, check) => sum + check.latencyMs, 0) / successful.length)
      : null
    ...
    const expiring = allMonitors.filter(
      (monitor) =>
        monitor.certificateExpiresAt && monitor.certificateExpiresAt <= certificateCutoff,
    )
    const attention = [
      ...allMonitors
        .filter((monitor) => monitor.status === "outage" || monitor.status === "degraded")
        .map((monitor) => ({
          type: monitor.status,
          id: `monitor:${monitor.id}`,
          monitorId: monitor.id,
          title: monitor.name,
          description: monitor.status === "outage" ? "服务当前中断" : "服务延迟偏高",
    ...
    return c.json({
      ...
      outageCount: enabled.filter((monitor) => monitor.status === "outage").length,
