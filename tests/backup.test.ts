...
describe("backup and restore", () => {
  ...
  test("rejects invalid pending restore and preserves the current database", async () => {
    const directory = await mkdtemp(
      path.join(tmpdir(), "monomi-invalid-restore-test-")
    )
    const config = parseConfig({ MONOMI_DATA_DIR: directory, NODE_ENV: "test" })
    let client = await openDatabase(config)
    try {
      insertMonitor(client, crypto.randomUUID(), "preserved")
      checkpointAndClose(client)
      await Bun.write(
        path.join(directory, "restore.pending.sqlite"),
        "not a database"
      )

      expect(await installPendingRestore(config)).toBe(false)
      client = await openDatabase(config)
      expect(
        client.db
          .select()
      ...
      expect(
        (await readdir(path.join(directory, "backups"))).some((name) =>
          name.startsWith("failed-restore-")
        )
      ).toBe(true)
    } finally {
      try {
