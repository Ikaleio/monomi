...
type HistoryDay = Extract<PublicStatusData, { enabled: true }>["monitors"][number]["history"][number]

const statusLabels: Record<HistoryDay["status"], "operational" | "degraded" | "outage" | "noData" | "maintenance"> = {
...
export function StatusHistory({ history, label }: { history: HistoryDay[]; label: string }) {
  const { t, i18n } = useTranslation()
  return (
    <div className="flex flex-col gap-3">
      <div className="overflow-x-auto pb-2" aria-label={`${label} 最近 90 天状态`}>
            ...
            <Tooltip key={day.date}>
              <TooltipTrigger asChild>
                <motion.button
                  ...
                  viewport={{ once: true }}
                  transition={{ delay: Math.min(index * 0.008, 0.56), duration: 0.56, type: "spring", bounce: 0.12 }}
                  whileHover={{ y: -5, scaleY: 1.12 }}
                />
              </TooltipTrigger>
              <TooltipContent side="top" className="flex min-w-52 flex-col items-start gap-1.5 p-3">
                <span className="font-medium">{formatPublicDate(day.date, i18n.language)}</span>
                <span>{t(statusLabels[day.status])}</span>
                <span className="text-background/75">
                  {day.uptime == null ? t("noData") : `${day.uptime.toFixed(2)}%`} · {day.checks} {t("checks")}
                </span>
              </TooltipContent>
            </Tooltip>
          ))}
