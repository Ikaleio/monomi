...
export default function MonitorDetailRoute() {
  ...
  async function runNow() {
    ...
    try {
      await fetchJson(
        api.admin.monitors[":id"].run.$url({ param: { id } }).toString(),
        { method: "POST" }
      )
      toast.success("检测已完成")
      await refreshAll()
  ...
  async function toggleEnabled() {
    setPending(true)
    try {
      const input = editableMonitor(monitor)
      input.enabled = !monitor.enabled
      await fetchJson(
        api.admin.monitors[":id"].$url({ param: { id } }).toString(),
        {
          method: "PATCH",
      ...
      toast.success(input.enabled ? "监视器已恢复" : "监视器已暂停")
      await refreshAll()
    } finally {
      setPending(false)
    }
  ...
  async function regenerateHeartbeat() {
    const result = await fetchJson<{ heartbeatPath: string }>(
      api.admin.monitors[":id"]["heartbeat-token"]
        .$url({ param: { id } })
        .toString(),
      { method: "POST" }
    )
    setHeartbeatUrl(`${window.location.origin}${result.heartbeatPath}`)
    toast.success("Heartbeat 地址已重新生成")
  }
    ...
    <main className="flex w-full flex-col gap-6 px-3 py-6 md:gap-7 md:px-8 md:py-8">
      <div className="flex flex-col justify-between gap-3 md:flex-row md:items-end">
        ...
        <div className="flex flex-wrap gap-2">
          ...
          <Button asChild>
            <Link to={`/app/monitors/${monitor.id}/edit`}>编辑</Link>
          </Button>
          <AlertDialog>
            ...
            <AlertDialogContent>
              <AlertDialogHeader>
                <AlertDialogTitle>删除监视器？</AlertDialogTitle>
                <AlertDialogDescription>
                  该操作会删除相关检查、故障和通知历史，无法撤销。
                </AlertDialogDescription>
              </AlertDialogHeader>
              <AlertDialogFooter>
          ...
          <CardContent className="flex flex-col gap-3">
            {heartbeatUrl && (
              <Alert>
                ...
                <AlertDescription>
                  <code className="block overflow-x-auto rounded-lg bg-muted p-3">
                    {heartbeatUrl}
                  </code>
                </AlertDescription>
              </Alert>
            ...
            <Button
              variant="outline"
              onClick={() => void regenerateHeartbeat()}
            >
              重新生成地址
            </Button>
          </CardContent>
      ...
      <div className="grid gap-4 xl:grid-cols-2">
        <Card>
          <CardHeader>
            ...
            <CardDescription>每五分钟聚合成功检查</CardDescription>
          </CardHeader>
          <CardContent>
            {buckets.length ? (
              <ChartContainer
        ...
        <Card>
          <CardHeader>
            <CardTitle className="font-serif text-2xl">90 天历史</CardTitle>
            <CardDescription>每日可用性状态</CardDescription>
          </CardHeader>
          <CardContent>
            {history.length ? (
      ...
      <Card>
        <CardHeader>
          <CardTitle className="font-serif text-2xl">最近检查</CardTitle>
          <CardDescription>
            最多显示最近 50 条，列表使用虚拟滚动。
          </CardDescription>
        </CardHeader>
        <CardContent>
          <Table>
              ...
              renderItem={(check) => (
                <div className="grid h-full grid-cols-4 items-center border-b px-2 text-sm">
                  ...
                  <span className="truncate text-muted-foreground">
                    {check.errorMessage ?? check.statusCode ?? "—"}
                  </span>
                </div>
              )}
            />
