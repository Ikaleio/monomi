    ...
    <main className="flex w-full flex-col gap-5 px-3 py-6 md:gap-6 md:px-8 md:py-8">
      <div className="flex flex-col justify-between gap-4 sm:flex-row sm:items-end">
        <div className="flex flex-col gap-2">
          ...
          <h1 className="font-serif text-4xl font-semibold">监视器</h1>
          <p className="text-muted-foreground">
            管理 HTTP、TCP 和 Heartbeat 检测目标。
          </p>
        </div>
        <Button asChild>
          <Link to="/app/monitors/new">新建监视器</Link>
        </Button>
      </div>
      <Card>
        <CardHeader>
          <CardTitle className="font-serif text-2xl">全部监视器</CardTitle>
          <CardDescription>{data.monitors.length} 个配置</CardDescription>
        </CardHeader>
        <CardContent>
          {data.monitors.length === 0 ? (
            <Empty className="min-h-64">
              <EmptyHeader>
                ...
                <EmptyDescription>创建第一个监视器开始检测。</EmptyDescription>
                <Button asChild>
                  <Link to="/app/monitors/new">新建监视器</Link>
                </Button>
              </EmptyHeader>
              ...
              renderItem={(monitor) => (
                ...
                >
                  <div className="flex min-w-0 items-center gap-3">
                    <span
                      className={`size-2 shrink-0 rounded-full ${monitor.status === "operational" ? "bg-primary" : monitor.status === "paused" ? "bg-foreground/30" : "bg-destructive"}`}
                      aria-hidden="true"
                    />
